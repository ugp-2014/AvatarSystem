...
const int MaxNum = 16;
const int Page = CResource::MaxBottomsNum / MaxNum;

CBottomsManager::CBottomsManager(std::shared_ptr<CTask> task) :
CPage(task, Page-1)
{
	...
	for (int i = 0; i < Page; i++)
	{
		...
		{
			parts.push_back(std::make_unique<CBottoms>(task, it, i,Float2(0, -100)));
		}
	}

}

void CBottomsManager::PartsDraw()
{
	const Float2 BackPos = Float2(50, 70);
	if (select_id <= -1) return;
	TextureAsset(CResource::GetInstance().GetBottoms(select_id)).draw(BackPos);
